...
static void compose_draw(struct _xrender_data *xd, const struct backend_image *img, const struct _xrender_image_data_inner *inner, bool has_alpha,
						 int dst_x1, int dst_y1, int tmpw, int tmph, int tmpew, int tmpeh, unsigned int alpha_pict, const struct region_t *reg,
						 const xcb_render_color_t dim_color, xcb_render_picture_t result)
{
...
#undef DOUBLE_TO_XFIXED
	...
	if (!inner->has_alpha || cr == 0) {
		...
		//log_warn("f(%d, %d) imge(%d %d) img(%d %d) dst(%d %d) a:%d",
		//	fullwid, fullhei, img->ewidth, img->eheight,
		//	img->width, img->height, dst_x, dst_y, img->has_alpha);
