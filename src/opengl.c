...
bool glx_init_dual_kawase_blur(session_t *ps) {
	assert(ps->o.blur_kernel_count > 0);
...
bool
glx_init_dualkawase_fbo_blur(session_t *ps) {
...
bool glx_init_blur(session_t *ps) {
	...
	case BLUR_METHOD_ALT_KAWASE:
		return glx_init_dualkawase_fbo_blur(ps);
 ...
 */
bool
glx_dualkawase_fbo_blur_dst(session_t *ps, int dx, int dy, int width, int height, float z attr_unused,
	...
	if (!psbc->fbos[0]) {
		...
		if (!psbc->fbos[0]) {
			log_error("Failed to generate Framebuffer. Cannot do "
						"multi-pass blur with GLX backend.");
			goto glx_dualkawase_fbo_blur_dst_end;
		}
...
glx_dualkawase_fbo_blur_dst_end:
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	glBindTexture(tex_tgt, 0);
	glDisable(tex_tgt);
	if (have_scissors)
...
bool glx_dual_kawase_blur_dst(session_t *ps, int dx, int dy, int width, int height, float z attr_unused,
	...
	if (!tex_scr) {
		log_error("Failed to allocate texture.");
		goto glx_dual_kawase_blur_dst_end;
	}
	for (int i = 1; i <= iterations; i++) {
		if (!pbc->textures[i]) {
...
bool glx_blur_dst(session_t *ps, int dx, int dy, int width, int height, float z,
  ...
  switch (ps->o.blur_method) {
	case BLUR_METHOD_DUAL_KAWASE:
		ret = glx_dual_kawase_blur_dst(ps, dx, dy, width, height, z,
									opacity, reg_tgt, pbc);
      break;
	case BLUR_METHOD_ALT_KAWASE:
		ret = glx_dualkawase_fbo_blur_dst(ps, dx, dy, width, height, z,
									opacity, reg_tgt, pbc);
      break;
    case BLUR_METHOD_KERNEL:
	case BLUR_METHOD_BOX:
